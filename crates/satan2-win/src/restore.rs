...
use windows_sys::Win32::{
    Foundation::*,
    System::{Com::*, LibraryLoader::*, Variant::*},
};
...
pub fn delete_all(verbose: bool) -> Result<u32, String> {
    ...
    for seq_num in restore_points {
        ...
        if ok != 0 {
            deleted += 1;
            if verbose {
                eprintln!("[+] Deleted restore point #{}", seq_num);
            }
        } else {
...
fn enumerate_restore_points(verbose: bool) -> Result<Vec<u32>, String> {
    unsafe {
        ...
        if hr < 0 {
            return Err(format!("CoCreateInstance: 0x{:x}", hr as u32));
        }
        ...
        (*locator).Release();
        if hr < 0 {
            return Err(format!("ConnectServer ROOT\\DEFAULT: 0x{:x}", hr as u32));
        }
        ...
        loop {
            ...
            if hr >= 0 && var.Anonymous.Anonymous.vt == VT_I4 {
