...
pub fn wipe_prefetch(verbose: bool) -> PrefetchStats {
    ...
    // Prevent future prefetch recording
    if disable_sysmain() {
        eprintln!("[+] prefetch: SysMain service disabled");
    }

    eprintln!(
        "[+] prefetch: {} file(s) deleted, {} KiB freed, {} error(s)",
        stats.files_deleted,
