...
pub fn wipe_lnk_jumplists(verbose: bool) -> LnkStats {
    ...
    let appdata =
        env::var("APPDATA").unwrap_or_else(|_| String::from(r"C:\Users\Default\AppData\Roaming"));
    ...
    delete_glob(
        &format!(
            r"{}\AutomaticDestinations\*.automaticDestinations-ms",
            recent
        ),
        &mut stats,
        true,
    ...
    if !local.is_empty() {
        delete_glob(
            &format!(
                r"{}\Microsoft\Windows\Recent\AutomaticDestinations\f01b4d95cf55d32a.automaticDestinations-ms",
                local
            ),
            &mut stats,
            true,
    ...
    if verbose {
        eprintln!(
            "[+] lnk: {} LNK + {} JumpList file(s) deleted, {} KiB freed",
            stats.lnk_deleted,
            stats.jumplist_deleted,
