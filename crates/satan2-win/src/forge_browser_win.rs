...
impl Lcg {
    fn new(seed: i64) -> Self {
        Lcg(seed as u64 ^ 0xb0bb_1e50_face_cafe)
    }
...
const URL_POOL: &[(&str, &str)] = &[
    ...
    (
        "https://google.com/search?q=csv+parsing+python",
        "csv parsing python - Google Search",
    ),
    (
        "https://stackoverflow.com/questions/2081586",
        "Python split string - Stack Overflow",
    ),
    (
...
fn chrome_ts(unix_ts: i64) -> i64 {
    (unix_ts + 11_644_473_600) * 1_000_000
}

// Firefox PRTime: microseconds since Unix epoch
fn ff_ts(unix_ts: i64) -> i64 {
    unix_ts * 1_000_000
}
...
fn inject_chrome(conn: &Connection, url: &str, title: &str, ts: i64, count: u64) -> bool {
    ...
    let uid: i64 = conn
        .query_row("SELECT id FROM urls WHERE url=?1", params![url], |r| {
            r.get(0)
        })
        .or_else(|_| {
            conn.execute(
            ...
            .ok();
            conn.query_row("SELECT id FROM urls WHERE url=?1", params![url], |r| {
                r.get(0)
            })
        })
        .unwrap_or(-1);
...
fn chromium_db_paths(user: &Path) -> Vec<PathBuf> {
...
fn firefox_db_paths(user: &Path) -> Vec<PathBuf> {
    ...
    for user in user_dirs() {
        ...
        for db_path in chromium_db_paths(&user) {
            let conn = match Connection::open(&db_path) {
                ...
                Err(e) => {
                    if opts.verbose {
                        eprintln!("[!] forge-browser-win: {:?}: {}", db_path, e);
                    }
                    s.errors += 1;
                    continue;
                }
            };
            if ensure_chrome_schema(&conn).is_err() {
                s.errors += 1;
                continue;
            }
            let mut ok = 0u32;
            for i in 0..opts.n_entries {
                let ts =
                    opts.ts_start + i as i64 * step + lcg.range(0, step.min(3600) as u64) as i64;
                let count = lcg.range(1, 20);
                let (url, title) = lcg.pick(URL_POOL);
                if inject_chrome(&conn, url, title, ts, count) {
                    ok += 1;
                }
            }
            s.chrome_entries += ok;
        ...
        for db_path in firefox_db_paths(&user) {
            ...
            if ensure_firefox_schema(&conn).is_err() {
                s.errors += 1;
                continue;
            }
            let mut ok = 0u32;
            for i in 0..opts.n_entries {
                ...
                let (url, title) = lcg.pick(URL_POOL);
                if inject_firefox(&conn, url, title, ts, count) {
                    ok += 1;
                }
            }
            ...
            if opts.verbose {
                eprintln!(
                    "[+] forge-browser-win: {} Firefox entries → {:?}",
                    ok, db_path
                );
            }
        }
