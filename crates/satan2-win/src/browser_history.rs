...
fn overwrite_and_delete(path: &Path, stats: &mut BrowserStats) {
    ...
    match fs::remove_file(path) {
        ...
        Err(e) => {
            if e.raw_os_error() != Some(32) { // ignore ERROR_SHARING_VIOLATION (open tab)
...
fn clean_chromium_vendor(vendor_dir: &Path, verbose: bool, stats: &mut BrowserStats) {
    ...
    for entry in entries.flatten() {
        ...
        if ns == "Default" || ns.starts_with("Profile ") || ns.starts_with("Guest ") {
            if verbose { eprintln!("[*] browser: chromium profile: {}", entry.path().display()); }
...
fn clean_gecko_profile(profile_dir: &Path, verbose: bool, stats: &mut BrowserStats) {
    if verbose { eprintln!("[*] browser: gecko profile: {}", profile_dir.display()); }
    ...
    for sub in &["datareporting", "crashes", "sessionstore-backups",
                  "storage", "thumbnails"] {
...
fn clean_gecko_vendor(appdata_roaming: &str, vendor: &str, product: &str,
                       verbose: bool, stats: &mut BrowserStats) {
...
pub fn wipe_browser_history(verbose: bool) -> BrowserStats {
    ...
    for user in users.flatten() {
        ...
        for (rel, name) in chromium_vendors {
            ...
            let p = Path::new(&dir);
            if p.exists() {
                if verbose { eprintln!("[*] browser: {} for {}", name, home.display()); }
        ...
        let ie_cache = format!(r"{}\Microsoft\Windows\WebCache\WebCacheV01.dat", local_appdata);
        if Path::new(&ie_cache).exists() {
            overwrite_and_delete(Path::new(&ie_cache), &mut stats);
        }
    }

    eprintln!("[+] browser: {} profile(s), {} file(s) deleted, {} MiB freed, {} error(s)",
        stats.profiles_cleaned, stats.files_deleted, stats.bytes_freed >> 20, stats.errors);
    stats
}
