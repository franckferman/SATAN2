...
use windows_sys::Win32::System::Registry::{
    RegOpenKeyExW, RegDeleteValueW, RegEnumValueW, RegQueryInfoKeyW,
    RegCreateKeyExW, RegSetValueExW, RegCloseKey,
    HKEY_CURRENT_USER, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, KEY_READ,
    REG_SZ,
};
...
struct Lcg(u64);
impl Lcg {
    fn new(seed: i64) -> Self { Lcg(seed as u64 ^ 0xfeed_face_cafe_1234) }
...
fn open_muicache_key(access: u32) -> Option<isize> {
    let path_w = to_wide(MUI_CACHE_PATH);
    let mut hkey: isize = 0;

    let ret = unsafe {
        RegOpenKeyExW(HKEY_CURRENT_USER, path_w.as_ptr(), 0, access, &mut hkey)
    };
...
pub fn wipe_muicache(verbose: bool) -> MuiCacheStats {
    ...
    let hkey = match open_muicache_key(KEY_ALL_ACCESS) {
        Some(k) => k,
        None => {
            if verbose { eprintln!("[!] wipe-muicache: failed to open key"); }
            s.errors += 1;
            return s;
        }
    };
    ...
    for name in &names {
        ...
        if !name.ends_with(".exe") && !name.ends_with(".exe.FriendlyAppName")
            && !name.ends_with(".exe.ApplicationCompany") { continue; }

        let name_w = to_wide(name);
        let ret = unsafe { RegDeleteValueW(hkey, name_w.as_ptr()) };
        if ret == ERROR_SUCCESS as i32 {
            s.values_deleted += 1;
            if verbose { eprintln!("[+] wipe-muicache: deleted '{}'", name); }
        } else {
            s.errors += 1;
        }
    }
...
const FAKE_MUI_ENTRIES: &[(&str, &str)] = &[
    ...
    (r"C:\Windows\System32\mmc.exe",          "Microsoft Management Console"),
...
pub fn forge_muicache(n_entries: u32, verbose: bool) -> MuiCacheStats {
    ...
    let ret = unsafe {
        RegCreateKeyExW(
            HKEY_CURRENT_USER, path_w.as_ptr(), 0,
    ...
    for i in 0..n {
        let (path, desc) = FAKE_MUI_ENTRIES[i];
        ...
        let ret = unsafe {
            RegSetValueExW(
                ...
                desc_w.as_ptr() as *const u8, data_bytes as u32,
            )
        };
        if ret == ERROR_SUCCESS as i32 {
            s.values_forged += 1;
            if verbose { eprintln!("[+] forge-muicache: {} = {}", path, desc); }
        } else {
            s.errors += 1;
        }
    }
