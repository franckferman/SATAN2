...
struct Lcg(u64);
impl Lcg {
    fn new(seed: i64) -> Self { Lcg(seed as u64 ^ 0xdead_beef_1234_abcd) }
...
fn compress_mam(raw: &[u8]) -> Option<Vec<u8>> {
    unsafe {
        ...
        if hmod == 0 { return None; }

        // GetProcAddress returns FARPROC = Option<unsafe extern "system" fn() -> isize>
        let gws_raw = GetProcAddress(hmod, b"RtlGetCompressionWorkSpaceSize\0".as_ptr())?;
        let cmp_raw = GetProcAddress(hmod, b"RtlCompressBuffer\0".as_ptr())?;
        ...
        if get_ws(XPRESS_HUFF_MAX, &mut ws_size, &mut frag_size) != 0 { return None; }
...
fn build_pf_inner(exe_name: &str, exe_path: &str, run_count: u32,
                  last_run_ts: u64, version: u32) -> Vec<u8> {
    ...
    for &wc in &wchars { buf.extend_from_slice(&wc.to_le_bytes()); }
    for _ in 0..(30 - wchars.len()) { buf.extend_from_slice(&0u16.to_le_bytes()); }
    ...
    for &wc in &vol_wchars { buf.extend_from_slice(&wc.to_le_bytes()); }
...
fn build_paths_section(path: &str) -> Vec<u8> {
    ...
    let mut sec = Vec::new();
    for &wc in &wchars { sec.extend_from_slice(&wc.to_le_bytes()); }
    sec.extend_from_slice(&0u16.to_le_bytes());
    sec
}

fn unix_to_filetime(unix_ts: i64) -> u64 {
    (unix_ts + 11_644_473_600) * 10_000_000
}

// ── Fake executables ──────────────────────────────────────────────────────────

const FAKE_EXES: &[(&str, &str)] = &[
    ("CHROME.EXE",       r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES (X86)\GOOGLE\CHROME\APPLICATION\CHROME.EXE"),
    ("FIREFOX.EXE",      r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\MOZILLA FIREFOX\FIREFOX.EXE"),
    ("OUTLOOK.EXE",      r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\MICROSOFT OFFICE\ROOT\OFFICE16\OUTLOOK.EXE"),
    ("WINWORD.EXE",      r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\MICROSOFT OFFICE\ROOT\OFFICE16\WINWORD.EXE"),
    ("EXCEL.EXE",        r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\MICROSOFT OFFICE\ROOT\OFFICE16\EXCEL.EXE"),
    ("POWERPNT.EXE",     r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\MICROSOFT OFFICE\ROOT\OFFICE16\POWERPNT.EXE"),
    ("TEAMS.EXE",        r"\DEVICE\HARDDISKVOLUME2\USERS\USER\APPDATA\LOCAL\MICROSOFT\TEAMS\CURRENT\TEAMS.EXE"),
    ("EXPLORER.EXE",     r"\DEVICE\HARDDISKVOLUME2\WINDOWS\EXPLORER.EXE"),
    ("MSPAINT.EXE",      r"\DEVICE\HARDDISKVOLUME2\WINDOWS\SYSTEM32\MSPAINT.EXE"),
    ("NOTEPAD.EXE",      r"\DEVICE\HARDDISKVOLUME2\WINDOWS\SYSTEM32\NOTEPAD.EXE"),
    ("POWERSHELL.EXE",   r"\DEVICE\HARDDISKVOLUME2\WINDOWS\SYSTEM32\WINDOWSPOWERSHELL\V1.0\POWERSHELL.EXE"),
    ("MSIEXEC.EXE",      r"\DEVICE\HARDDISKVOLUME2\WINDOWS\SYSTEM32\MSIEXEC.EXE"),
    ("WUAUCLT.EXE",      r"\DEVICE\HARDDISKVOLUME2\WINDOWS\SYSTEM32\WUAUCLT.EXE"),
    ("SVCHOST.EXE",      r"\DEVICE\HARDDISKVOLUME2\WINDOWS\SYSTEM32\SVCHOST.EXE"),
    ("ONEDRIVE.EXE",     r"\DEVICE\HARDDISKVOLUME2\USERS\USER\APPDATA\LOCAL\MICROSOFT\ONEDRIVE\ONEDRIVE.EXE"),
    ("SKYPE.EXE",        r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES (X86)\MICROSOFT\SKYPE FOR DESKTOP\SKYPE.EXE"),
    ("ACROBAT.EXE",      r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\ADOBE\ACROBAT DC\ACROBAT\ACROBAT.EXE"),
    ("SLACK.EXE",        r"\DEVICE\HARDDISKVOLUME2\USERS\USER\APPDATA\LOCAL\SLACK\SLACK.EXE"),
    ("CODE.EXE",         r"\DEVICE\HARDDISKVOLUME2\USERS\USER\APPDATA\LOCAL\PROGRAMS\MICROSOFT VS CODE\CODE.EXE"),
    ("WINZIP32.EXE",     r"\DEVICE\HARDDISKVOLUME2\PROGRAM FILES\WINZIP\WINZIP32.EXE"),
];
...
pub fn forge_prefetch(opts: &PrefetchForgeOpts) -> PrefetchForgeStats {
    ...
    if !pf_dir.exists() {
        if let Err(e) = fs::create_dir_all(&pf_dir) {
            if opts.verbose { eprintln!("[!] forge-prefetch: create dir: {}", e); }
            s.errors += 1;
            return s;
        }
    }
    ...
    for i in 0..n {
        let (exe_name, exe_path) = FAKE_EXES[i];
        ...
        let (pf_data, is_mam) = {
            ...
            match compress_mam(&raw_v30) {
                Some(mam) => (mam, true),
                None      => (build_pf_inner(exe_name, exe_path, run_count, last_run_ts, 23), false),
            }
        };
        ...
        match fs::OpenOptions::new().write(true).create(true).truncate(true).open(&pf_path) {
            Ok(mut f) => match f.write_all(&pf_data) {
                Ok(_) => {
                    s.files_written += 1;
                    if is_mam { s.mam_compressed += 1; }
                    if opts.verbose {
                        eprintln!("[+] forge-prefetch: wrote {:?} ({})",
                            pf_path, if is_mam { "v30 MAM" } else { "v23 fallback" });
                    }
                }
                Err(e) => {
                    s.errors += 1;
                    if opts.verbose { eprintln!("[!] forge-prefetch: write {:?}: {}", pf_path, e); }
                }
            },
            Err(e) => {
                s.errors += 1;
                if opts.verbose { eprintln!("[!] forge-prefetch: open {:?}: {}", pf_path, e); }
            }
        }
    }
    s
