...
pub fn wipe_usn_journal(verbose: bool) -> NtfsStats {
    ...
    for letter in b'C'..=b'Z' {
        ...
        match result {
            Ok(out) if out.status.success() => {
                ...
                if verbose { eprintln!("[+] ntfs: $UsnJrnl deleted on {}", vol); }
            }
            Ok(out) => {
                let stderr = String::from_utf8_lossy(&out.stderr);
                let stdout = String::from_utf8_lossy(&out.stdout);
                let msg = if !stderr.is_empty() { &*stderr } else { &*stdout };
                if verbose { eprintln!("[!] ntfs: USN delete on {}: {}", vol, msg.trim()); }
                s.errors += 1;
            }
            Err(e) => {
                if verbose { eprintln!("[!] ntfs: fsutil exec on {}: {}", vol, e); }
                s.errors += 1;
            }
        }
    }
    ...
    if verbose {
        eprintln!("[+] ntfs: {} volume(s) scanned, {} journal(s) deleted, {} errors",
