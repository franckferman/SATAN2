...
fn delete_dir_contents(dir: &str, ext_filter: Option<&str>, stats: &mut EtwStats, is_wer: bool) {
    ...
    for entry in WalkDir::new(dir).follow_links(false).into_iter().flatten() {
        ...
        match fs::remove_file(path) {
            Ok(()) => {
                if is_wer { stats.wer_deleted += 1; }
                else       { stats.etl_deleted += 1; }
            }
            Err(e) => {
...
pub fn wipe_etw(verbose: bool) -> EtwStats {
    ...
    for dir in &etl_dirs {
        if verbose { eprintln!("[*] etw: cleaning {}", dir); }
    ...
    for dir in &wer_dirs {
        if verbose { eprintln!("[*] etw: clearing WER {}", dir); }
    ...
    if let Ok(users) = fs::read_dir(r"C:\Users") {
        for user in users.flatten() {
            let wer = format!(r"{}\AppData\Local\Microsoft\Windows\WER",
                user.path().display());
    ...
    eprintln!("[+] etw: {} ETL + {} WER file(s) deleted, {} MiB freed, {} error(s)",
        stats.etl_deleted, stats.wer_deleted, stats.bytes_freed >> 20, stats.errors);
    stats
}
