...
struct Lcg(u64);
impl Lcg {
    fn new(seed: i64) -> Self { Lcg(seed as u64 ^ 0xf00d_cafe_1234_5678) }
    ...
    fn b64_string(&mut self, n: usize) -> String {
        ...
        (0..n).map(|_| A[(self.next() % 64) as usize] as char).collect()
    }
}
...
fn open_journal_socket() -> Option<RawFd> {
    if !Path::new(JOURNAL_SOCKET).exists() { return None; }
    unsafe {
...
fn send_journal_entry(fd: RawFd, msg: &str) -> bool {
    ...
    unsafe {
        libc::send(fd, bytes.as_ptr() as *const libc::c_void, bytes.len(), 0) >= 0
    }
}
...
const SERVICES: &[&str] = &[
    "nginx", "rsyslog", "cron", "ntp", "snapd", "udisksd", "networkd-dispatcher",
];
...
pub fn forge_journal(opts: &JournalForgeOpts) -> JournalForgeStats {
    ...
    let fd = match open_journal_socket() {
        Some(f) => f,
        None    => {
            if opts.verbose { eprintln!("[!] forge-journal: {} not available", JOURNAL_SOCKET); }
            s.errors += 1;
            return s;
        }
    };
    ...
    for i in 0..opts.n_entries {
        ...
        for m in &msgs {
            if send_journal_entry(fd, m) { s.entries_sent += 1; }
            else { s.errors += 1; }
        }
        ...
        if lcg.range(0, 3) < 2 {
            ...
            if send_journal_entry(fd, &m) { s.entries_sent += 1; } else { s.errors += 1; }
        }
    ...
    if opts.verbose { eprintln!("[+] forge-journal: {} entries sent", s.entries_sent); }
    s
}
