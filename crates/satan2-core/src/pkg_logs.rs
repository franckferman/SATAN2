...
fn remove_file(path: &str, stats: &mut PkgLogsStats, verbose: bool) {
    ...
    match fs::remove_file(p) {
        Ok(()) => {
            if verbose {
                eprintln!("[*] pkg-logs: rm {}", path);
            }
            stats.files_wiped += 1;
            stats.bytes_freed += size;
        }
        Err(e) => {
            eprintln!("[!] pkg-logs: {}: {}", path, e);
            stats.errors += 1;
        }
    }
}
...
fn truncate_file(path: &str, stats: &mut PkgLogsStats, verbose: bool) {
        ...
        .and_then(|f| f.set_len(0))
    {
        Ok(()) => {
            if verbose {
                eprintln!("[*] pkg-logs: truncated {}", path);
            }
            stats.files_wiped += 1;
            stats.bytes_freed += size;
        }
        Err(e) => {
            eprintln!("[!] pkg-logs: truncate {}: {}", path, e);
            stats.errors += 1;
        }
    }
}
...
fn remove_dir(path: &str, stats: &mut PkgLogsStats, verbose: bool) {
    ...
    match fs::remove_dir_all(p) {
        Ok(()) => {
            if verbose {
                eprintln!("[*] pkg-logs: rmdir {}", path);
            }
            stats.dirs_cleared += 1;
        }
        Err(e) => {
            eprintln!("[!] pkg-logs: rmdir {}: {}", path, e);
            stats.errors += 1;
        }
    }
}
...
pub fn wipe_pkg_logs(verbose: bool) -> PkgLogsStats {
    ...
    for home in &homes {
        // pip logs
        remove_glob(
            &format!("{}/.local/share/pip/*.log", home),
            &mut stats,
            verbose,
        );
        ...
        // Go module cache
        remove_dir(
            &format!("{}/.cache/go/pkg/mod/cache", home),
            &mut stats,
            verbose,
        );
    ...
    eprintln!(
        "[+] pkg-logs: {} file(s) ({} MiB), {} dir(s), {} error(s)",
        stats.files_wiped,
