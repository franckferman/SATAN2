...
impl Lcg {
    fn new(seed: i64) -> Self {
        Lcg(seed as u64 ^ 0x1234_abcd_5678_ef00)
    }
...
fn gen_pacman_lines(pkg: &str, ver: &str, ts: i64, lcg: &mut Lcg) -> Vec<String> {
    ...
    vec![
        format!(
            "{} [PACMAN] Running 'pacman -S --noconfirm {}'",
            t_start, pkg
        ),
...
pub fn forge_pkg_logs(opts: &PkgForgeOpts) -> PkgForgeStats {
        ...
        || Path::new("/var/log/dpkg.log").exists()
    {
        ...
        for i in 0..opts.n_packages {
            ...
            apt_lines.extend(gen_apt_history_block(
                pkg,
                ver,
                ts,
                ts + lcg.range(2, 15) as i64,
            ));
        }

        for (path, lines) in [
            ("/var/log/dpkg.log", &dpkg_lines),
            ...
            match append_lines(path, lines) {
                Ok(n) => {
                    ...
                    s.files_touched += 1;
                    if opts.verbose {
                        eprintln!("[+] forge-pkg: {} lines → {}", n, path);
                    }
                }
                Err(e) => {
                    s.errors += 1;
                    if opts.verbose {
                        eprintln!("[!] forge-pkg: {}", e);
                    }
                }
            }
        }
            ...
            .unwrap_or(false)
    {
        ...
        match append_lines("/var/log/pacman.log", &lines) {
            Ok(n) => {
                ...
                s.files_touched += 1;
                if opts.verbose {
                    eprintln!("[+] forge-pkg: {} lines → /var/log/pacman.log", n);
                }
            }
            Err(e) => {
                s.errors += 1;
                if opts.verbose {
                    eprintln!("[!] forge-pkg pacman: {}", e);
                }
            }
        }
    }
                ...
                .unwrap_or(false)
        {
            ...
            match append_lines(dnf_path, &lines) {
                Ok(n) => {
                    ...
                    s.files_touched += 1;
                    if opts.verbose {
                        eprintln!("[+] forge-pkg: {} lines → {}", n, dnf_path);
                    }
                }
                Err(e) => {
                    s.errors += 1;
                    if opts.verbose {
                        eprintln!("[!] forge-pkg dnf: {}", e);
                    }
                }
            }
        }
        ...
        if let Ok(rd) = std::fs::read_dir(home_base) {
            for entry in rd.flatten() {
                ...
                for _ in 0..n_pip {
                    ...
                    lines.push(format!(
                        "VERBOSE: Found existing installation: {} 0.0.0",
                        pkg
                    ));
                    lines.push(format!("VERBOSE: Uninstalling {} 0.0.0:", pkg));
                    lines.push(format!("INFO: Successfully installed {}-2.31.0", pkg));
                }
                if let Some(p) = pip_log.parent() {
                    let _ = std::fs::create_dir_all(p);
                }
                if let Ok(mut f) = OpenOptions::new().append(true).create(true).open(&pip_log) {
                    for l in &lines {
                        let _ = writeln!(f, "{}", l);
                    }
