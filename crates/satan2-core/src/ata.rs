...
const ATA16_TFLAGS_PIO_OUT: u8 = (0 << 4) | (1 << 3) | 2;
...
fn ata_do(cmd: &mut AtaCmd) -> Result<()> {
    ...
    let hdr = SgIoHdr {
        ...
        dxferp:          cmd.data.map(|p| p as *mut libc::c_void).unwrap_or(std::ptr::null_mut()),
        cmdp:            cmd.cdb.as_ptr(),
    ...
    let r = unsafe {
        libc::ioctl(cmd.fd, SG_IO, &hdr as *const SgIoHdr)
    };
    if r < 0 {
        return Err(format!("SG_IO ioctl failed: errno={}", unsafe { *libc::__errno_location() }));
    }
    if hdr.status != 0 {
...
pub fn ata_secure_erase(dev_path: &str) -> Result<()> {
    ...
    eprintln!("[*] Security: supported={} enabled={} locked={} frozen={}",
        caps.supported, caps.enabled, caps.locked, caps.frozen);

    if !caps.supported {
        return Err(format!("{}: ATA Security Feature Set not supported", dev_path));
    }

    if caps.frozen {
        return Err(
            "Drive is security-frozen (BIOS SECURITY FREEZE LOCK).\n\
    ...
    eprintln!("[*] Erase timeout: {}s ({}min estimate, enhanced={})",
