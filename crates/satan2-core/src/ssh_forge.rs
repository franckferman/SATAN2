...
struct Lcg(u64);
impl Lcg {
    fn new(seed: u32) -> Self { Lcg(seed as u64 ^ 0xfeed_face_dead_beef) }
    ...
    fn b64(&mut self, n: usize) -> String { (0..n).map(|_| self.b64char()).collect() }
...
const RFC1918_PREFIXES: &[&str] = &[
    "10.0.0.", "10.0.1.", "10.0.2.", "10.10.0.", "10.10.1.",
...
fn append_known_hosts(kh_path: &Path, lines: &[String]) -> Result<usize, String> {
    ...
    let mut f = OpenOptions::new()
        .append(true).create(true).open(kh_path)
    ...
    for home in &homes {
        ...
        match append_known_hosts(&kh, &lines) {
            Ok(n)  => {
                ...
                s.files_touched += 1;
                if opts.verbose { eprintln!("[+] forge-ssh: {} entries → {:?}", n, kh); }
            }
            Err(e) => {
                s.errors += 1;
                if opts.verbose { eprintln!("[!] forge-ssh: {}", e); }
            }
        }
    }
