...
pub fn list_swap() -> Result<Vec<SwapEntry>> {
    ...
    for (i, line) in BufReader::new(f).lines().enumerate() {
        ...
        if !path.is_empty() {
            entries.push(SwapEntry { path, kind, size_kb, priority });
        }
    }
    Ok(entries)
}
...
fn wipe_area(path: &str, size_bytes: u64, is_partition: bool) -> Result<()> {
    ...
    let mut f = if is_partition {
        ...
        if fd < 0 {
            return Err(format!("open {}: errno={}", path, unsafe { *libc::__errno_location() }));
        }
        // Wrap fd in a File-like for write operations
        unsafe { use std::os::unix::io::FromRawFd; std::fs::File::from_raw_fd(fd) }
    } else {
        OpenOptions::new()
            .write(true)
            .open(path)
    ...
    while written < size_bytes {
        ...
        let chunk = if is_partition { chunk & !(512 - 1) } else { chunk };
        if chunk == 0 { break; }
        ...
        eprint!("\r[>] swap wipe {}: {}/{} MiB",
            path, written >> 20, size_bytes >> 20);
    }
    eprintln!();
    Ok(())
}
...
pub fn wipe_swap_entry(entry: &SwapEntry, reenable: bool) -> Result<()> {
    ...
    if reenable {
        let flags = SWAP_FLAG_PREFER | ((entry.priority & SWAP_FLAG_PRIO_MASK) << SWAP_FLAG_PRIO_SHIFT);
        let r = unsafe { libc::swapon(path_c.as_ptr(), flags) };
        if r < 0 {
            eprintln!("[!] swapon {} failed: errno={}", entry.path,
                unsafe { *libc::__errno_location() });
        } else {
