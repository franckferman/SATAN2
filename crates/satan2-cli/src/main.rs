...
use satan2_core::{
    ...
    fs_kill::{fs_kill_partition_table, fs_kill_filesystem},
    meta::{meta_process, MetaOpts, TsStrategy, SigStrategy},
    slack::{slack_wipe_dir, slack_wipe_free, SlackStats},
    log_poison::{log_poison, LogpOpts, LogpMode, LogpReplace, LogpStats},
    swap::wipe_all_swap,
    ...
    lastlog_forge::{wipe_lastlog, forge_lastlog},
};

#[cfg(target_os = "linux")]
use satan2_crypto::{
    ops::{create_container, encrypt_file, encrypt_dir, decrypt_container,
          encrypt_device, destroy_and_encrypt},
...
impl Report {
    fn new(dry_run: bool) -> Self { Self { dry_run, modules: Vec::new() } }
    ...
    fn push(&mut self, module: &'static str, res: Result<(), String>) {
        ...
        self.modules.push(ModResult { module, ok: res.is_ok(), msg: res.err() });
    }

    fn push_ok(&mut self, module: &'static str) {
        eprintln!("[+] {}: ok", module);
        self.modules.push(ModResult { module, ok: true, msg: None });
    }

    fn print_json(&self) {
        let mods: Vec<String> = self.modules.iter().map(|m| {
            let msg_field = m.msg
                .as_ref()
                .map(|s| format!(",\"msg\":\"{}\"",
                    s.replace('\\', "\\\\").replace('"', "\\\"")))
                .unwrap_or_default();
            format!("{{\"module\":\"{}\",\"ok\":{}{}}}", m.module, m.ok, msg_field)
        }).collect();
        println!("{{\"dry_run\":{},\"modules\":[{}]}}", self.dry_run, mods.join(","));
    }
}
...
#[derive(Parser)]
#[command(name = "satan2", about = "SATAN2 — disk sanitization and counter-forensics (Linux)")]
struct Cli {
...
enum Cmd {
    ...
    LogPoison {
        ...
        replace: Vec<String>,
        #[arg(long)] auth:    bool,
        #[arg(long)] syslog:  bool,
        #[arg(long)] wtmp:    bool,
        #[arg(long)] history: bool,
        #[arg(long)] journal: bool,
        #[arg(long)]
        extra: Vec<String>,
    },
...
enum WipeAlgoArg { Random, Dod, Schneier, Gutmann }

#[derive(ValueEnum, Clone)]
enum TsArg { RandomPlausible, RandomFull, Epoch, Clone }

#[derive(ValueEnum, Clone)]
enum ModeArg { Cover, Destroy }
...
#[derive(ValueEnum, Clone, Debug)]
enum HashAlgoArg { Sha256, Sha512, Blake2b, Sha3256, Sha3512 }
...
fn run() {
    ...
    match cli.command {
        ...
        Cmd::DestroyAll { device, reenable_audit } => {
        ...
        Cmd::NvmeErase { device } => {
            ...
            if !dry { report.push("nvme-erase", NvmeDev::secure_erase(&device)); }
            else     { report.push_ok("nvme-erase [dry-run]"); }
        }

        Cmd::AtaErase { device } => {
            eprintln!("[*] ATA secure erase: {}", device);
            if !dry { report.push("ata-erase", ata_secure_erase(&device)); }
            else     { report.push_ok("ata-erase [dry-run]"); }
        }

        Cmd::Wipe { device, algo, verify } => {
            let algo = match algo {
            ...
            let opts = WipeOpts { algo, verify_last: verify, verbose: true };
            eprintln!("[*] Wipe: {} ({:?})", device, opts.algo);
            if !dry { report.push("wipe", wipe_device(&device, &opts)); }
            else     { report.push_ok("wipe [dry-run]"); }
        }

        Cmd::KillPt { device } => {
            eprintln!("[*] Kill partition table: {}", device);
            if !dry { report.push("kill-pt", fs_kill_partition_table(&device)); }
            else     { report.push_ok("kill-pt [dry-run]"); }
        }

        Cmd::KillFs { device } => {
            eprintln!("[*] Kill filesystem: {}", device);
            if !dry { report.push("kill-fs", fs_kill_filesystem(&device)); }
            else     { report.push_ok("kill-fs [dry-run]"); }
        }

        Cmd::Swap { reenable } => {
            eprintln!("[*] Wipe swap...");
            if !dry { report.push("swap", wipe_all_swap(reenable)); }
            else     { report.push_ok("swap [dry-run]"); }
        }
        ...
        Cmd::Meta { path, recursive, ts, ts_ref, sig_mask, xattrs } => {
            let ts_strategy = map_ts(ts);
            let opts = MetaOpts {
                do_timestamps: true, do_sig_mask: sig_mask, do_xattrs: xattrs,
                recursive, ts_strategy, ts_clone_ref: ts_ref,
                sig_strategy: SigStrategy::Random, verbose: true,
            };
            eprintln!("[*] Meta: {}", path);
            if !dry { report.push("meta", meta_process(&path, &opts)); }
            else     { report.push_ok("meta [dry-run]"); }
        }

        Cmd::Slack { dir, recursive, free } => {
            eprintln!("[*] Slack wipe...");
            if !dry {
                let mut s = SlackStats::default();
                let mut res = Ok(());
                if let Some(d) = dir { res = slack_wipe_dir(&d, recursive, &mut s); }
                if res.is_ok() {
                    if let Some(mp) = free { res = slack_wipe_free(&mp, &mut s); }
                }
        ...
        Cmd::LogPoison { mode, replace, auth, syslog, wtmp, history, journal, extra } => {
            let logp_mode = match mode { ModeArg::Cover => LogpMode::Cover, ModeArg::Destroy => LogpMode::Destroy };
            let replacements = parse_replacements(&replace);
            let opts = LogpOpts {
                mode: logp_mode, replacements,
                scramble_ts: false, ts_window_start: 0, ts_window_end: 0,
                do_auth_log: auth, do_syslog: syslog, do_wtmp: wtmp,
                do_bash_history: history, do_journal: journal,
                extra_logs: extra, verbose: true,
            };
            if !dry {
        ...
        Cmd::SshClean { hosts, destroy_all, wipe_keys, wipe_config } => {
        ...
        Cmd::ForgeLog { fake_ip, fake_user, n_sessions, ts_start, ts_end, fake_hostname, auth, syslog, bash } => {
            let now = unsafe { libc::time(std::ptr::null_mut()) };
            let ts_e = ts_end.unwrap_or(now);
            let ts_s = ts_start.unwrap_or(ts_e - 86_400);
            eprintln!("[*] ForgeLog: {} sessions, IP={}, user={}", n_sessions, fake_ip, fake_user);
            ...
            if !dry {
                let opts = LogForgeOpts {
                    fake_ip, fake_user, n_sessions, ts_start: ts_s, ts_end: ts_e,
                    fake_hostname, do_auth: auth, do_syslog: syslog,
                    bash_history_paths: bash_paths, verbose: true,
                };
                let s = forge_logs(&opts);
                if s.errors > 0 {
                    report.push("forge-log", Err(format!("{} error(s)", s.errors)));
                } else {
                    eprintln!("[+] forge-log: {} lines written, {} files", s.lines_written, s.files_touched);
        ...
        Cmd::ForgePkgLogs { n_packages, ts_start, ts_end } => {
            let now = unsafe { libc::time(std::ptr::null_mut()) };
            let ts_e = ts_end.unwrap_or(now);
            let ts_s = ts_start.unwrap_or(ts_e - 172_800); // 48h
            eprintln!("[*] ForgePkgLogs: {} packages", n_packages);
            if !dry {
                let opts = PkgForgeOpts { n_packages, ts_start: ts_s, ts_end: ts_e, verbose: true };
                let s = forge_pkg_logs(&opts);
                eprintln!("[+] forge-pkg-logs: {} lines, {} files", s.lines_written, s.files_touched);
                if s.errors > 0 {
        ...
        Cmd::ForgeSsh { hosts, n_random, home } => {
            eprintln!("[*] ForgeSsh: {} explicit hosts + {} random", hosts.len(), n_random);
            if !dry {
                let opts = SshForgeOpts { hosts, n_random, user_homes: home, verbose: true };
                let s = forge_ssh(&opts);
                eprintln!("[+] forge-ssh: {} entries, {} files", s.entries_added, s.files_touched);
                if s.errors > 0 {
        ...
        Cmd::ForgeBrowser { n_urls, ts_start, ts_end, profile } => {
            let now = unsafe { libc::time(std::ptr::null_mut()) };
            let ts_e = ts_end.unwrap_or(now);
            ...
            if !dry {
                let opts = BrowserForgeOpts {
                    n_urls, ts_start: ts_s, ts_end: ts_e,
                    profile_path: profile, verbose: true,
                };
                let s = forge_browser_history(&opts);
                eprintln!("[+] forge-browser: {} URLs, {} DBs", s.urls_injected, s.dbs_touched);
                if s.errors > 0 {
        ...
        Cmd::ForgeWtmp { fake_ip, fake_user, n_sessions, ts_start, ts_end } => {
            let now = unsafe { libc::time(std::ptr::null_mut()) };
            let ts_e = ts_end.unwrap_or(now);
            let ts_s = ts_start.unwrap_or(ts_e - 172_800);
            eprintln!("[*] ForgeWtmp: {} sessions, IP={}, user={}", n_sessions, fake_ip, fake_user);
            if !dry {
                let opts = WtmpForgeOpts { fake_ip, fake_user, n_sessions, ts_start: ts_s, ts_end: ts_e, verbose: true };
                let s = forge_wtmp(&opts);
                eprintln!("[+] forge-wtmp: {} records written, {} files, {} errors",
            ...
            } else { report.push_ok("forge-wtmp [dry-run]"); }
        }

        Cmd::ForgeJournal { fake_ip, fake_user, n_entries, ts_start, ts_end } => {
            let now = unsafe { libc::time(std::ptr::null_mut()) };
            let ts_e = ts_end.unwrap_or(now);
            let ts_s = ts_start.unwrap_or(ts_e - 172_800);
            eprintln!("[*] ForgeJournal: {} entries, IP={}, user={}", n_entries, fake_ip, fake_user);
            if !dry {
                let opts = JournalForgeOpts { fake_ip, fake_user, n_entries, ts_start: ts_s, ts_end: ts_e, verbose: true };
                let s = forge_journal(&opts);
                eprintln!("[+] forge-journal: {} entries sent, {} errors", s.entries_sent, s.errors);
                if s.errors > 0 {
                    report.push("forge-journal", Err(format!("{} error(s)", s.errors)));
                } else {
                    report.push_ok("forge-journal");
                }
            } else { report.push_ok("forge-journal [dry-run]"); }
        }

        Cmd::SecureDelete { targets, passes } => {
            eprintln!("[*] SecureDelete: {} target(s), {} passes", targets.len(), passes);
            if !dry {
                let s = secure_delete_targets(&targets, passes, true);
                eprintln!("[+] secure-delete: {} files, {} bytes, {} errors",
            ...
            } else { report.push_ok("secure-delete [dry-run]"); }
        }
        ...
        Cmd::MemoryWipe => {
            ...
            if !dry {
                ...
                eprintln!("[+] memory-wipe: {} MiB zeroed, cache_dropped={}", s.bytes_zeroed / 1_048_576, s.cache_dropped);
                report.push_ok("memory-wipe");
            } else { report.push_ok("memory-wipe [dry-run]"); }
        }
        ...
        Cmd::ProcClean => {
            ...
            if !dry {
                ...
                eprintln!("[+] proc-clean: {} files, {} dirs, {} errors",
            ...
            } else { report.push_ok("proc-clean [dry-run]"); }
        }
        ...
        Cmd::DockerCover => {
            ...
            if !dry {
                ...
                eprintln!("[+] docker-cover: {} logs, {} configs, {} dirs, {} errors",
            ...
            } else { report.push_ok("docker-cover [dry-run]"); }
        }

        Cmd::SelfAudit { findings_json: emit_json } => {
            let findings = audit_artifacts(true);
            if emit_json {
                let items: Vec<String> = findings.iter().map(|f| {
                    format!(
            ...
            eprintln!("[+] self-audit: {} findings ({} HIGH, {} MEDIUM)", findings.len(), high, med);
            if findings.is_empty() {
                report.push_ok("self-audit");
            } else {
                report.push("self-audit", Err(format!("{} residual artifacts detected", findings.len())));
            }
        }
        ...
        Cmd::WipeLastlog => {
            ...
            if !dry {
                ...
                eprintln!("[+] wipe-lastlog: {} zeroed, {} errors", s.entries_wiped, s.errors);
                if s.errors > 0 {
                    report.push("wipe-lastlog", Err(format!("{} error(s)", s.errors)));
                } else {
                    report.push_ok("wipe-lastlog");
                }
            } else { report.push_ok("wipe-lastlog [dry-run]"); }
        }
        ...
        Cmd::ForgeLastlog { ts_start, ts_end } => {
            ...
            if !dry {
                ...
                eprintln!("[+] forge-lastlog: {} forged, {} errors", s.entries_forged, s.errors);
                if s.errors > 0 {
                    report.push("forge-lastlog", Err(format!("{} error(s)", s.errors)));
                } else {
                    report.push_ok("forge-lastlog");
                }
            } else { report.push_ok("forge-lastlog [dry-run]"); }
        }
        ...
        Cmd::Timestomp { path, ts } => {
                ...
                if parts.len() == 2 {
                    ...
                    if date.len() == 3 && time.len() >= 2 {
                        ...
                        let days = (y - 1970) * 365 + (y - 1969) / 4 - (y - 1901) / 100 + (y - 1601) / 400
                            ...
                            + d - 1;
                        days * 86_400 + time[0] as i64 * 3600 + time[1] as i64 * 60
            ...
            if !dry {
                let tp = libc::timespec { tv_sec: unix_ts, tv_nsec: 0 };
            ...
            } else { report.push_ok("timestomp [dry-run]"); }
        }

        Cmd::ForgeAll { fake_ip, fake_user, n, ts_start, ts_end } => {
            let now = unsafe { libc::time(std::ptr::null_mut()) };
            let ts_e = ts_end.unwrap_or(now);
            let ts_s = ts_start.unwrap_or(ts_e - 172_800); // 48h
            eprintln!("[*] FORGE ALL — Linux (n={}, IP={}, user={})", n, fake_ip, fake_user);
            if !dry {
                // 1. Log injection
                let mut bash_paths = vec!["/root/.bash_history".to_string()];
                if let Ok(rd) = std::fs::read_dir("/home") {
                    for e in rd.flatten() {
                        bash_paths.push(e.path().join(".bash_history").to_string_lossy().to_string());
                    }
                }
                let log_opts = LogForgeOpts {
                    fake_ip: fake_ip.clone(), fake_user: fake_user.clone(),
                    n_sessions: n, ts_start: ts_s, ts_end: ts_e,
                    fake_hostname: None, do_auth: true, do_syslog: true,
                    bash_history_paths: bash_paths, verbose: false,
                };
                let ls = forge_logs(&log_opts);
                eprintln!("[+] forge-all: log → {} lines, {} files", ls.lines_written, ls.files_touched);

                // 2. Package manager logs
                let pkg_opts = PkgForgeOpts { n_packages: n + 5, ts_start: ts_s, ts_end: ts_e, verbose: false };
                let ps = forge_pkg_logs(&pkg_opts);
                eprintln!("[+] forge-all: pkg → {} lines, {} files", ps.lines_written, ps.files_touched);

                // 3. SSH known_hosts
                let ssh_opts = SshForgeOpts { hosts: vec![], n_random: n / 2 + 3, user_homes: vec![], verbose: false };
                let ss = forge_ssh(&ssh_opts);
                eprintln!("[+] forge-all: ssh → {} entries, {} files", ss.entries_added, ss.files_touched);

                // 4. Browser history
                let br_opts = BrowserForgeOpts { n_urls: n + 10, ts_start: ts_s, ts_end: ts_e, profile_path: None, verbose: false };
                let bs = forge_browser_history(&br_opts);
                eprintln!("[+] forge-all: browser → {} URLs, {} DBs", bs.urls_injected, bs.dbs_touched);

                // 5. wtmp / utmp binary records
                let wt_opts = WtmpForgeOpts {
                    fake_ip: fake_ip.clone(), fake_user: fake_user.clone(),
                    n_sessions: n / 2 + 2, ts_start: ts_s, ts_end: ts_e, verbose: false,
                };
                let wt = forge_wtmp(&wt_opts);
                eprintln!("[+] forge-all: wtmp → {} records, {} files", wt.records_written, wt.files_touched);

                // 6. systemd journal entries
                let jn_opts = JournalForgeOpts {
                    fake_ip: fake_ip.clone(), fake_user: fake_user.clone(),
                    n_entries: n, ts_start: ts_s, ts_end: ts_e, verbose: false,
                };
                let jn = forge_journal(&jn_opts);
                eprintln!("[+] forge-all: journal → {} entries sent", jn.entries_sent);

                let total_errors = ls.errors + ps.errors + ss.errors + bs.errors + wt.errors + jn.errors;
                if total_errors > 0 {
        ...
        Cmd::CreateContainer { output, size_mib, algo, kdf_iters, pass_env } => {
            let pass = require_passphrase(&pass_env);
            let size = size_mib * 1024 * 1024;
            eprintln!("[*] Create container: {} ({} MiB, {:?})", output, size_mib, algo);
            if !dry {
                report.push("create-container",
        ...
        Cmd::EncryptFile { input, output, algo, kdf_iters, pass_env } => {
            let pass = require_passphrase(&pass_env);
            eprintln!("[*] Encrypt file: {} → {}", input, output);
            if !dry {
                match encrypt_file(&input, &output, map_crypto_algo(algo), &pass, kdf_iters) {
                    Ok(b)  => { eprintln!("[+] encrypt-file: {} bytes", b); report.push_ok("encrypt-file"); }
        ...
        Cmd::EncryptDir { input, output, algo, kdf_iters, pass_env } => {
            let pass = require_passphrase(&pass_env);
            eprintln!("[*] Encrypt dir: {} → {}", input, output);
            if !dry {
                match encrypt_dir(&input, &output, map_crypto_algo(algo), &pass, kdf_iters) {
                    Ok(b)  => { eprintln!("[+] encrypt-dir: {} bytes", b); report.push_ok("encrypt-dir"); }
        ...
        Cmd::Decrypt { input, output, pass_env } => {
            let pass = require_passphrase(&pass_env);
            eprintln!("[*] Decrypt: {} → {}", input, output);
            if !dry {
                match decrypt_container(&input, &output, &pass) {
                    Ok(b)  => { eprintln!("[+] decrypt: {} bytes", b); report.push_ok("decrypt"); }
        ...
        Cmd::Extract { input, output, pass_env } => {
            let pass = require_passphrase(&pass_env);
            eprintln!("[*] Extract container: {} → {}/", input, output);
            if !dry {
                match extract_dir(&input, &output, &pass) {
                    Ok(b)  => { eprintln!("[+] extract: {} bytes", b); report.push_ok("extract"); }
        ...
        Cmd::EncryptDevice { device, algo, kdf_iters, pass_env } => {
            let pass = require_passphrase(&pass_env);
            eprintln!("[*] Encrypt device in-place: {} ({:?}) — IRREVERSIBLE", device, algo);
            if !dry {
                report.push("encrypt-device",
        ...
        Cmd::DestroyAndEncrypt { sources, output, algo, kdf_iters, pass_env } => {
            if sources.is_empty() {
                eprintln!("[!] destroy-and-encrypt: no sources specified");
                std::process::exit(1);
            }
            let pass = require_passphrase(&pass_env);
            eprintln!("[*] Destroy-and-encrypt: {} source(s) → {}", sources.len(), output);
            if !dry {
                let src_refs: Vec<&str> = sources.iter().map(String::as_str).collect();
                match destroy_and_encrypt(&src_refs, &output, map_crypto_algo(algo), &pass, kdf_iters) {
                    Ok(b)  => { eprintln!("[+] destroy-and-encrypt: {} bytes, sources wiped", b); report.push_ok("destroy-and-encrypt"); }
        ...
        Cmd::Hash { path, algo } => {
            ...
            match hash_file(&path, halgo) {
                ...
                Err(e)     => { eprintln!("[!] hash: {}", e); std::process::exit(1); }
            }
            return; // no report entry for pure query commands
        }
    }

    if json { report.print_json(); }
}
...
fn parse_replacements(pairs: &[String]) -> Vec<LogpReplace> {
    pairs.iter().filter_map(|s| {
        let mut p = s.splitn(2, ':');
        let n = p.next()?.to_string();
        let r = p.next().unwrap_or("").to_string();
        Some(LogpReplace { needle: n, replacement: r })
    }).collect()
}
...
fn get_passphrase(env_var: &str) -> Option<Vec<u8>> {
    if let Ok(s) = std::env::var(env_var) {
        if !s.is_empty() { return Some(s.into_bytes()); }
    }
...
fn require_passphrase(env_var: &str) -> Vec<u8> {
    match get_passphrase(env_var) {
        Some(p) => p,
        None    => { eprintln!("[!] could not obtain passphrase from '{}' or terminal", env_var); std::process::exit(1); }
    }
}
...
fn prompt_passphrase_noecho(env_var: &str) -> Result<Vec<u8>, String> {
    ...
    if buf.is_empty() { Err("passphrase cannot be empty".to_string()) } else { Ok(buf) }
}
...
fn run_destroy_all(dry: bool, reenable_audit: bool, device: Option<&str>, report: &mut Report) {
    ...
    if !dry {
        let opts = LogpOpts {
            mode: LogpMode::Destroy, replacements: vec![],
            scramble_ts: false, ts_window_start: 0, ts_window_end: 0,
            do_auth_log: true, do_syslog: true, do_wtmp: true,
            do_bash_history: true, do_journal: true,
            extra_logs: vec![], verbose: true,
        };
        let mut s = LogpStats::default();
        report.push("log-poison", log_poison(&opts, &mut s));
    } else {
    ...
    if !dry {
        let opts = SshCleanOpts {
            hosts: &[], key_fragments: &[],
            destroy_all: true, wipe_keys: true, wipe_config: true,
        };
        let mut s = SshCleanStats::default();
        report.push("ssh-clean", ssh_clean(&opts, &mut s));
    } else {
    ...
    for (dir, label) in dirs {
        if std::path::Path::new(dir).exists() {
            if !dry {
                let opts = MetaOpts {
                    do_timestamps: true, do_sig_mask: false, do_xattrs: true,
                    recursive: true, ts_strategy: TsStrategy::RandomPlausible,
                    ts_clone_ref: None, sig_strategy: SigStrategy::Random,
                    verbose: false,
                };
    ...
    // 9. Swap
    eprintln!("\n[*] == Swap ==");
    if !dry { report.push("swap", wipe_all_swap(false)); }
    else     { report.push_ok("swap [dry-run]"); }
    ...
    if let Some(dev) = device {
        ...
        if !dry {
            let r = NvmeDev::secure_erase(dev)
                ...
                .or_else(|_| {
                    let opts = WipeOpts { algo: WipeAlgo::Random, verify_last: false, verbose: true };
...
fn run_cover_all(dry: bool, replace: &[String], hosts: &[String], report: &mut Report) {
    ...
    if !dry {
        let replacements = parse_replacements(replace);
        let opts = LogpOpts {
            mode: LogpMode::Cover, replacements,
            scramble_ts: false, ts_window_start: 0, ts_window_end: 0,
            do_auth_log: true, do_syslog: true, do_wtmp: true,
            do_bash_history: true, do_journal: true,
            extra_logs: vec![], verbose: true,
    ...
    if !dry {
        let host_refs: Vec<&str> = hosts.iter().map(String::as_str).collect();
        let opts = SshCleanOpts {
            hosts: &host_refs, key_fragments: &[],
            destroy_all: false, wipe_keys: false, wipe_config: false,
        };
        let mut s = SshCleanStats::default();
        report.push("ssh-clean", ssh_clean(&opts, &mut s));
    } else {
