...
impl ContainerReader {
    fn new(file: std::fs::File, engine: Engine, hdr: &CvHeader) -> Self {
        ...
        let n_sectors = (data_len + SECTOR_SIZE as u64 - 1) / SECTOR_SIZE as u64;
        ContainerReader {
            file, engine, n_sectors, data_len,
            sector: 0,
            buf: [0u8; SECTOR_SIZE],
            buf_pos: 0,
            buf_end: 0,
...
impl Read for ContainerReader {
    fn read(&mut self, out: &mut [u8]) -> std::io::Result<usize> {
        ...
        if self.buf_pos >= self.buf_end {
            if self.sector >= self.n_sectors { return Ok(0); }
...
pub fn extract_dir(src: &str, dst_dir: &str, passphrase: &[u8]) -> Result<u64, String> {
    ...
    fs::create_dir_all(dst_dir)
        .map_err(|e| format!("create dir '{}': {}", dst_dir, e))?;
...
fn parse_inline_tar<R: Read>(reader: &mut R, dst_dir: &str) -> Result<u64, String> {
    ...
    loop {
        ...
        reader.read_exact(&mut path_bytes).map_err(|e| e.to_string())?;
...
fn skip_bytes<R: Read>(r: &mut R, mut n: u64) -> Result<(), String> {
    ...
    while n > 0 {
        ...
        r.read_exact(&mut buf[..to_read]).map_err(|e| e.to_string())?;
        n -= to_read as u64;
    }
    Ok(())
}
